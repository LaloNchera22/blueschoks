...
import { LayoutGrid, Settings, Brush, Home, Copy, ExternalLink, Globe, Check, X, Crown } from "lucide-react"
...
export function AppSidebar({
  ...
  // Width logic separated for Mobile (Fixed Overlay) and Desktop (Relative Flow)
  // Mobile: Always fixed h-[100dvh], toggles via translate-x
  const mobileClasses = `fixed inset-y-0 left-0 z-50 h-[100dvh] w-64 shadow-2xl transform transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full"}`

  // Desktop: Relative, h-full, toggles via width
  const desktopClasses = `md:relative md:translate-x-0 md:h-full md:shadow-none md:transition-all md:duration-300 ${isOpen ? "md:w-64" : "md:w-0 md:border-none md:overflow-hidden"}`
  ...
  return (
    <>
      ...
      <aside className={`${mobileClasses} ${desktopClasses} bg-white border-r border-slate-200 flex flex-col shrink-0 overflow-y-auto custom-scrollbar`}>

        {/* 1. HEADER */}
        {/* Compact padding on mobile: p-4 vs p-6 */}
        <div className="p-4 pb-2 md:p-6 md:pb-2 relative group">
          {/* Close Button */}
          {isOpen && onClose && (
             <button
               onClick={onClose}
          ...
          {/* Compact margin on mobile: mb-3 vs mb-6 */}
        ...
        <div className="px-4 space-y-1 flex-1 md:flex-none">
           ...
           <Link href="/dashboard/products" className={getItemClass("/dashboard/products")} prefetch={false}>
              <LayoutGrid size={18} strokeWidth={1.5} /> <span>Mis Productos</span>
           </Link>

           <Link href="/dashboard/design" className={getItemClass("/dashboard/design")} prefetch={false}>
              <Brush size={18} strokeWidth={1.5} /> <span>Diseño</span>
           </Link>
        ...
        {/* 3. TU TIENDA ONLINE */}
        {/* Compact margin on mobile: mt-4 vs mt-8 */}
        <div className="px-4 mt-4 md:mt-8">
           ...
           <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 shadow-sm">
              ...
              >
                 {copied ? (
                     <div className="flex items-center"><Check size={12} strokeWidth={1.5} className="mr-2"/> Link Copiado</div>
                 ) : (
                     <div className="flex items-center"><Copy size={12} strokeWidth={1.5} className="mr-2"/> {hasShopUrl ? "Copiar Link" : "Configurar Link"}</div>
                 )}
              </Button>
